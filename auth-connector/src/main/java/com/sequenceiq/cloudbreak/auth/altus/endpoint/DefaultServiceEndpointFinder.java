...
import com.sequenceiq.cloudbreak.endpoint.DirectServiceEndpointFinder;
...
public class DefaultServiceEndpointFinder implements ServiceEndpointFinder {
    ...
    private final int cacheExpirationInSeconds = 600;
    ...
    public DefaultServiceEndpointFinder(@Nonnull Supplier<ServiceEndpoint> minasshdGrpcEndpointSupplier, @Nullable Cache<String, Object> cache) {
        ...
        if (cache == null) {
            cache = CacheBuilder.newBuilder()
                    .expireAfterAccess(cacheExpirationInSeconds, TimeUnit.SECONDS)
                    .build();
        }
        this.cache = cache;
    }
