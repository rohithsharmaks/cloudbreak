...
import com.sequenceiq.cloudbreak.endpoint.HostEndpoint;
import com.sequenceiq.cloudbreak.endpoint.KnownServiceIdentifier;
...
public class MinaSshdServiceEndpointFinder implements ServiceEndpointFinder {
    ...
    static {
        ...
        map.put(KnownServiceIdentifier.NGINX, MinaSshdProto.EndpointService.Value.NGINX);
    ...
    private static final Set<Status.Code> RETRYABLE_STATUS_CODES =
            Collections.unmodifiableSet(EnumSet.of(Status.Code.UNAVAILABLE, Status.Code.NOT_FOUND));
    ...
    public MinaSshdServiceEndpointFinder(String lookupHost, int lookupPort) {
        if ((lookupHost == null) || lookupHost.isEmpty()) {
            throw new IllegalArgumentException("Lookup host is required.");
        }

        if (lookupPort < MIN_PORT || lookupPort > MAX_PORT) {
            throw new IllegalStateException("Invalid lookup port, not in [1,65535].");
        }
