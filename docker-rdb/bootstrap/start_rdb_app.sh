...
echo "Starting the RDB application..."

set -x
if [ "$SECURE_RANDOM" == "false" ]; then
  RDB_JAVA_OPTS="$RDB_JAVA_OPTS -Djava.security.egd=file:/dev/./urandom"
fi

eval "(java $RDB_JAVA_OPTS -jar /rdb.jar) & JAVAPID=\$!; trap \"kill \$JAVAPID; wait \$JAVAPID\" SIGINT SIGTERM; wait \$JAVAPID"
