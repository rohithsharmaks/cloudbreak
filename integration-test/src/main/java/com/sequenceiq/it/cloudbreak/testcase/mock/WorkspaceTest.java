...
public class WorkspaceTest extends AbstractIntegrationTest {
    ...
    public void testCreateAnImageCatalogWithImagesAndGetOtherUser(TestContext testContext) {
        testContext
                ...
                .when(imageCatalogTestClient.getImagesByNameV4(), RunningParameter.key(FORBIDDEN_KEY).withWho(CloudbreakTest.SECONDARY_REFRESH_TOKEN).withLogError(false))
                .expect(ForbiddenException.class, RunningParameter.key(FORBIDDEN_KEY))
                .validate();
    }
    ...
    public void testCreateAnImageCatalogWithoutImagesAndGetOtherUser(TestContext testContext) {
        testContext
                .given(ImageCatalogTestDto.class)
                .when(imageCatalogTestClient.createV4())
                .when(imageCatalogTestClient.getV4(Boolean.FALSE), RunningParameter.key(FORBIDDEN_KEY).withWho(CloudbreakTest.SECONDARY_REFRESH_TOKEN).withLogError(false))
                .expect(ForbiddenException.class, RunningParameter.key(FORBIDDEN_KEY))
                .validate();
    }
